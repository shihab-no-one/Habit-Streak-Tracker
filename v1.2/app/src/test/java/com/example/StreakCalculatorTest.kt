...
class StreakCalculatorTest {
    ...
    @Test
    fun `CASE 9 - frozen day preserves active streak across breaks`() {
        // Sep 19: COMPLETED, Sep 20: FROZEN, Sep 21: COMPLETED, Sep 22 (today): COMPLETED
        val createdAt = today.minusDays(3)
        val statusMap = mapOf(
            today.minusDays(3) to com.example.data.model.CheckStatus.COMPLETED,
            today.minusDays(2) to com.example.data.model.CheckStatus.FROZEN,
            today.minusDays(1) to com.example.data.model.CheckStatus.COMPLETED,
            today to com.example.data.model.CheckStatus.COMPLETED
        )

        val result = StreakCalculator.calculateStreakWithStatus(
            createdAt = createdAt,
            getStatusOnDate = { statusMap[it] },
            today = today
        )

        // FROZEN day bridges the streak, so total completed streak days = 3
        assertEquals(3, result)
    }

    @Test
    fun `CASE 10 - partial completion preserves active streak with partial credit`() {
        // Sep 21: PARTIAL, Sep 22: COMPLETED
        val createdAt = today.minusDays(1)
        val statusMap = mapOf(
            today.minusDays(1) to com.example.data.model.CheckStatus.PARTIAL,
            today to com.example.data.model.CheckStatus.COMPLETED
        )
